...
obj_data, obj_v_count, obj_min, obj_max, tex_path = ObjLoader.load_model("planet.obj")
...
def process_input(window):
    ...
    if glm.length(move_dir) > 0:
        ...
        if obj_data is not None and check_exact_collision_fast(camera_pos, obj_data, radius=collision_radius):
            # Try sliding along right direction
            tried = False
            # attempt move along right only
            if not check_exact_collision_fast(prev_pos + move_right * glm.length(disp), obj_data, radius=collision_radius):
                camera_pos = prev_pos + move_right * glm.length(disp)
                tried = True
            # attempt move along forward only
            if not tried and not check_exact_collision_fast(prev_pos + move_forward * glm.length(disp), obj_data, radius=collision_radius):
                camera_pos = prev_pos + move_forward * glm.length(disp)
                tried = True
            if not tried:
                # revert
                camera_pos = prev_pos
...
# Updated Vertex Shader
    ...
    glUniform1i(has_tex_location, 1 if tex_id else 0)
    ...
    while not glfw.window_should_close(window):
        ...
        glBindVertexArray(obj_vao)
        glDrawArrays(GL_TRIANGLES, 0, obj_v_count)
